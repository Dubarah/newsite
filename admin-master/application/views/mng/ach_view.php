	            ...
	            <div class="col-sm-6">
	                <select name="search_by">
	                	...
	                	<!-- <option value="verified" selected="">Verified</option> -->
	                		<option value="all" selected="">All Achievements</option>
	                </select>
	            </div>
	        </div>
        	...
        	<table id="example" class="display table dataTable" style="width: 100%; cellspacing: 0;">
				...
				<tbody>
					...
					<tr>
						...
						<td style="text-align: center"><?php echo $result->created_date; ?></td>
						<td style="text-align: center"><?php echo $result->username.' '.$result->lastname; ?></td>
						
						<td style="text-align: center"><?php echo $result->title; ?></td>
						<td style="text-align: center"><?php echo $result->ach_title; ?></td>
						<td style="text-align: center"><?php echo $result->country_english_name; ?></td>
						
						<!-- <td>
							<button type="button" class="btn btn-success" data-toggle="modal" data-target=".bs-example-modal<?php echo $result->ach_id ?>"><i style="font-size: 20px" class="fa fa-bar-chart"></i></button>
                            <div class="modal fade bs-example-modal<?php echo $result->ach_id ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
						...
						<td id="stat<?php echo $result->ach_id ?>"><center>
							...
							<?php } ?>
								...
								<td id="verified<?php echo $result->ach_id ?>"><center>
						...
						<td><center>
							<a onclick="approve(<?php echo $result->ach_id ?>)" href="#">
								<i style="color: green; font-size: 20px; font-weight: bold" class="fa icon-like"></i>
							</a></center>
						</td>	
						<td><center>
							<a onclick="disapprove(<?php echo $result->ach_id ?>)" href="#">
								<i class="fa icon-dislike" style="font-size: 20px; font-weight: bold"></i>
							</a></center>
						</td>
						<td><center>
							<a onclick="verified(<?php echo $result->ach_id ?>)" href="#">
								<i style="color: green; font-size: 20px; font-weight: bold" class="fa icon-like"></i>
							</a></center>
						</td>
						<td><center>
							<a onclick="notverified(<?php echo $result->ach_id ?>)" href="#">
								<i class="fa icon-dislike" style="font-size: 20px; font-weight: bold"></i>
							</a></center>
						</td>
						</tr>
...
<script type="text/javascript">
	...
	function approve (ad_id) {
	  $.ajax({
			url: '<?php echo base_url(); ?>approve_ach/' + ad_id,
			success: function(data) {
				if (data == 1) {
					toastr["success"]("Achievement approve success")
					toastr.options = {
					  "closeButton": false,
					  "debug": false,
					  "newestOnTop": false,
	...
	function verified (ad_id) {
	  $.ajax({
			url: '<?php echo base_url(); ?>verified_ach/' + ad_id,
			success: function(data) {
				if (data == 1) {
					toastr["success"]("Achievement verified success")
					toastr.options = {
					  "closeButton": false,
					  "debug": false,
					  "newestOnTop": false,
	...
	function notverified (ad_id) {
	  $.ajax({
			url: '<?php echo base_url(); ?>notverified_ach/' + ad_id,
			success: function(data) {
				if (data == 1) {
					toastr["success"]("Achievement Not verified success")
					toastr.options = {
					  "closeButton": false,
					  "debug": false,
					  "newestOnTop": false,
	...
	function disapprove (ad_id) {
	  $.ajax({
			url: '<?php echo base_url(); ?>delete_ach/' + ad_id,
	        
			success: function(data) {
				if (data == 1) {
					toastr["success"]("Achievement deleted success")
					toastr.options = {
					  "closeButton": false,
					  "debug": false,
					  "newestOnTop": false,
